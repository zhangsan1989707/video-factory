...
- `2026-06-12`：`node tests/test_console_static_app.js`
- 回归覆盖：
  - `test_create_single_project_vertical_job_records_repo_and_plan_stage`
  - `test_create_single_project_vertical_job_rejects_bad_repo_without_directory`
  - `test_prepare_single_project_vertical_plan_uses_existing_pipeline`
  - `test_render_single_project_vertical_uses_from_plan_and_finalizes_output`
  - `testCreateSingleProjectDraftUsesRepoUrl`
...
- `2026-06-12`：`.venv/bin/python -m pytest tests/test_console_scheduler.py -q`
- `2026-06-12`：`node tests/test_console_static_app.js`
- 回归覆盖：
  - `test_auto_script_schedule_selects_candidates_and_waits_for_script_confirmation`
  - `test_failed_auto_script_schedule_does_not_mark_day_as_run`
  - `test_scheduler_config_normalizes_invalid_fields`
...
- `2026-06-13`：`.venv/bin/python -m pytest tests/test_real_e2e_smoke.py -q` 默认显示 skipped
- `2026-06-13`：完整轻量验证命令通过，覆盖控制台任务、调度、服务端 smoke、hotlist、from-plan、compileall、Node 静态测试和 `git diff --check`
- 新增 `docs/e2e-manual-checklist.md`，记录真实 API、浏览器采集、TTS、ffmpeg/ffprobe 的手动验收步骤
...
| 日期 | 更新 | 证据 |
| --- | --- | --- |
| 2026-06-13 | 完成 P2-3：新增真实端到端手动验收清单和显式慢速 e2e 测试，固定公共仓库样例并用环境变量控制真实链路运行 | `.venv/bin/python -m pytest tests/test_real_e2e_smoke.py -q` 默认 skipped；完整轻量验证命令通过 |
| 2026-06-12 | 完成 P2-2：定时任务新增 `candidates_only` / `auto_script` 模式，默认安全只生成候选；自动脚本模式会选前 N 个候选并生成口播草稿，仍等待人工确认且不会自动渲染 | `.venv/bin/python -m pytest tests/test_console_scheduler.py -q` ; `node tests/test_console_static_app.js` |
| 2026-06-12 | 完成 P2-1：控制台新增 `single_project_vertical` 任务类型，可输入仓库 URL 创建非热榜任务，并复用现有计划、校验、渲染、产物与历史版本机制 | `.venv/bin/python -m pytest tests/test_console_jobs.py -q` ; `node tests/test_console_static_app.js` |
